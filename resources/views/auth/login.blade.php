    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div class="flex items-center justify-between pt-1 px-2">
            ...
            @if(Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-xs font-black italic uppercase tracking-widest text-blue-600 hover:text-blue-700 transition-colors">
                    Lupa?
                </a>
            @endif
        </div>
        ...
        <div class="text-center pt-4">
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">
                Belum terdaftar? 
                <a href="{{ route('register') }}" class="text-blue-600 hover:underline inline-flex items-center gap-1 ml-1">
